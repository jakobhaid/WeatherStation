...
void sendDataToServer() {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("http://jkobh.org/sensor/sensor.php"); // URL des Servers
    http.addHeader("Content-Type", "application/json");

    // JSON-Daten erstellen
    JSONVar jsonData;
    jsonData["temperature68"] = temp68;
    jsonData["humidity68"] = hum68;
    jsonData["pressure68"] = pres68;
    jsonData["gas68"] = gas68;
    jsonData["altitude68"] = alt68;
    jsonData["temperature28"] = temp28;
    jsonData["pressure28"] = pres28;
    jsonData["temperatureWeb"] = tempWeb.toFloat();
    jsonData["humidityWeb"] = humWeb.toFloat();
    jsonData["pressureWeb"] = presWeb.toFloat();
    String jsonString = JSON.stringify(jsonData);

    // Überprüfen, ob JSON korrekt erstellt wurde
    if (jsonString.length() == 0) {
      Serial.println("Fehler beim Erstellen des JSON-Strings");
      return;
    }
